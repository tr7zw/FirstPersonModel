package dev.tr7zw.firstperson.fabric.features.hat;
...
public class ItemHatFeatureRenderer<T extends LivingEntity, M extends EntityModel<T> & HeadedModel>
		extends
			RenderLayer<T, M> {
	...
	public ItemHatFeatureRenderer(RenderLayerParent<T, M> featureRendererContext) {
		this(featureRendererContext, 1.0F, 1.0F, 1.0F);
	}

	public ItemHatFeatureRenderer(RenderLayerParent<T, M> featureRendererContext, float f, float g, float h) {
	...
	private ItemStack endrod = Items.END_ROD.getDefaultInstance();
	private ItemStack feather = Items.FEATHER.getDefaultInstance();

	public void render(PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int i, T livingEntity,
			float f, float g, float h, float j, float k, float l) {
		ItemStack itemStack = ItemStack.EMPTY;
		if(SettingsUtil.hasEnabled((AbstractClientPlayer)livingEntity, Hat.ENDROD)) {
			itemStack = endrod;
		}else if(SettingsUtil.hasEnabled((AbstractClientPlayer)livingEntity, Hat.FEATHER)) {
			itemStack = feather;
		}
